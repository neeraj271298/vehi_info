...
function populate(s1,s2)
{
		...
		s2.innerHTML="";
		if(s1.value=="kenworth"){
			var optionArray=["t680|T 680","k370|K 370","w900|W 900","t980|T 980"];
			
		}
		else if(s1.value=="volvo"){
			var optionArray=["fh16|FH 16","fm|FM Truck","fh4|FH 4 Axle"];
		}
		else if(s1.value=="peterbuilt"){
			var optionArray=["model567|Model 567","model389|Model 389"];
		}
...
<form action="truck2.php" method="post">
...
if(isset($_POST['submit'])){
	...
	$len=strlen($vin);
	if($len==17){
		$query="INSERT into vehicle_truck(username,company,model,fuel,vin,number_plate) VALUES('$user','$company','$model','$fuel','$vin',' ')";
		$res=mysqli_query($con,$query);
		if($res)
		{
		echo'<script>window.location="number_truck.php";</script>';
		}
		else{
			echo 'retry';
		}
