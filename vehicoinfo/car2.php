...
function populate(s1,s2)
{
		...
		s2.innerHTML="";
		if(s1.value=="suzuki"){
			var optionArray=["swift|Swift","baleno|Baleno","alto|Alto","equator|Equator"];
			
		}
		else if(s1.value=="datsun"){
			var optionArray=["go|Go","goplus|Go Plus","redigo|Redi Go"];
		}
		else if(s1.value=="porche"){
			var optionArray=["macan|Macan","boxster|Boxster","gt3|911 GT3","carrera|Carrera"];
		}
		else if(s1.value=="jaguar"){
			var optionArray=["xj|XJ","xj220|XJ 220","xk120|XK 120","ftype|F-Type"];
		}
		else if(s1.value=="lambogini"){
		var optionArray=["diablo|Diablo","gallardo|Gallardo","revention|Revention","egoista|Egoista"];
		}
...
<p style="opacity:0.5">(e.g. 4S4BRCCC3E1293878)</p>
...
if(isset($_POST['submit'])){
	...
	$len=strlen($vin);
	if($len==17){
		$query="INSERT into vehicle_car(username,company,model,fuel,vin,number_plate) VALUES('$user','$company','$model','$fuel','$vin',' ')";
		$res=mysqli_query($con,$query);
		if($res)
		{$_SESSION['car']='car';
		echo'<script>window.location="number_car.php";</script>';
		}
		else{
			echo 'retry';
		}
